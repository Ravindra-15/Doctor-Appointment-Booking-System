  ...
  try {
    const res = await fetch(`${BASE_URL}/users/${userData._id}`, {
      ...
      headers: {
        ...
        Authorization: `Bearer ${localStorage.getItem("token")}`, // if your API is protected
      },
    });
    ...
    dispatch({ type: "LOGOUT" }); // logout the user
  } catch (err) {
        ...
        {!loading && !error && <div className="grid md:grid-cols-3 gap-10">
          <div className="pb-[50px] px-[30px] rounded-md">
            ...
            <div className="text-center mt-4">
              ...
              <p className="text-textColor text-[15px] leading-6 font-medium">
                Blood Type :<span className=" ml-2 text-headingColor text -[22] leading-8">
                 {userData.bloodType}
                  </span>
              </p>
            </div>
...
>
  Delete account
          ...
          font-semibold text-[16px] leading-7 border border-solid border-primaryColor`}>
              ...
              <button onClick={() => setTab("settings")} className={`${tab === "settings" && "bg-primaryColor text-white font-normal"} p-2 mr-5 px-5 rounded-md text-headingColor font-semibold 
